<template>
  ...
  >
    ...
    <template v-if="data">
      <div v-for="product in data" :key="product.id" class="flex items-center justify-start gap-x-4 py2">
        <div><img width="48" heigh="48" src="../assets/img/iphone14.png" /></div>
        <div class="flex flex-col gap-y-1">
          <h2>{{ product.name }}</h2>
          <h3>{{ product.sell_price ? `$ ${product.sell_price}` : '-' }}</h3>
        </div>
...
defineProps<{ data: ProductTable[] | null }>()
</script>
